...
interface Props {
  ...
  override?: () => JSX.Element | undefined;
  ...
  /** The last source. */
  fallbackCode?: string;
  /* The function that wraps the text, if present. If nothing is displayed then this is not called. */
 ...
 *   - if specified, but returns an empty value, nothing is displayed.
 *   - if missing or returns undefined, continue:
 ...
 *   - if missing or empty, nothing is displayed.
 *
 ...
 *
 *     <TranslatedText ... wrapIn={text => <label>{text}</label>} />
 *
...
export const TranslatedText: Component<Props> = (props) => {
  const mProps = mergeProps({capitalize: false, wrapIn: (text) => text} satisfies Props, props);
  ...
  return (
    <Show
      ...
      fallback={
        <Show
          when={langFunc()}
          fallback={<Show when={mProps.fallbackCode}>{mProps.wrapIn(<>{mProps.fallbackCode}</>)}</Show>}
        >
          {(langFunc) => (
    ...
    >
      {(override) => <Show when={!override().empty}>{mProps.wrapIn(<>{override().value}</>)}</Show>}
    </Show>
  );
};
