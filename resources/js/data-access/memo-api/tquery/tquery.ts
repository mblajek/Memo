...
export const DEFAULT_REQUEST_CREATOR: RequestCreator<SetStoreFunction<DataRequest>> =
  schema => {
    ...
    createComputed(() => {
      ...
      if (sch) {
        ...
        setRequest({
          columns: (sch.suggestedColumns ? [...sch.suggestedColumns] :
            sch.columns.map(({name}) => name))
...
export function createTQuery<T>(entityURL: EntityURL,
  {requestCreator, dataQueryOptions}: {
    requestCreator: RequestCreator<T>,
    dataQueryOptions?: CreateQueryOptions<
      DataResponse, Api.Error, DataResponse, DataQueryKey>,
  }) {
  const schemaQueryKey: SchemaQueryKey = ["tquery", entityURL, "schema"];
  const fetchSchema: QueryFunction<Schema, SchemaQueryKey> = context =>
    V1.get<Schema>(`${context.queryKey[1]}/tquery`)
      .then(res => res.data);
  const schemaQuery = createQuery<
    Schema, Api.Error, Schema, SchemaQueryKey>(() => ({
  ...
  const dataQuery = createQuery<
    DataResponse, Api.Error, DataResponse, DataQueryKey>(() => ({
      ...
      queryFn: context => V1.post<DataResponse>(
        `${context.queryKey[1]}/tquery`, context.queryKey[3])
        .then(res => res.data),
      enabled: !!request(),
  ...
  const data = createMemo(on(() => dataQuery.dataUpdatedAt,
